(function() {
  ...
  function typeahead($window, $timeout ,d3Service) {
    return {
      template: '<input type="text" class="typeahead-input" placeholder="{{placeHolder}}"' +
      'ng-class="inputClass"' +
      'ng-model="searchTerm"' +
      'ng-keydown="keyDown($event)"' +
      'ng-blur="onBlur()" />' +

      '<div class="typeahead-options-container">' +
      '<div class="typeahead-options-dropdown" ng-if="showOptions">' +
      '<div class="typeahead-option" ng-if="!hasMatches">' +
      '<span>No matches</span>' +
      '</div>' +

      '<ul class="typeahead-options-list">' +
      '<li class="typeahead-option" ng-class="{selected: isOptionSelected(option)}" ' +
      ...
      controller: function($scope){
       ...
       /* $scope.keyDown = function(e) {
        ...
        $scope.$on('simple-typeahead:clearInput', function() {
          $scope.searchTerm = '';
        });
      ...
      link: function(scope, elem, attrs) {
        ...
        var inputElement = elem.children('.typeahead-input')[0];
  ...
  /*.factory('typeahead-keys', function() {
    return {
      upArrow: 38,
      downArrow: 40,
      enter: 13,
